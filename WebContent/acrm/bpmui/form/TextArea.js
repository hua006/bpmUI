...
(function($) {
	// 自定义多选框插件
	var TextArea = function(ele, opt, _parentCom) {
		this.$parent = $(ele);
	...
	TextArea.prototype = $.extend({}, Arvato.BaseComponent, {
		// 设置或获取控件值
		val : function(value) {
		...
		_initializeElement:function (){
			...
			var $field = $(formatStr('<textarea name="{0}"></textarea>', this.settings.name)).appendTo($fieldDiv);
			this.$me = $field;
	...
	// 在jQuery对象上添加构造方法
	// 定义插件
	$.fn.TextArea = function(options, _parentCom) {
		
		// 使用 默认值
		var opts = $.extend({}, $.fn.TextArea.defaults, options);
		
		// 创建插件对象
		var comp = new TextArea(this, opts, _parentCom);
