...
(function($) {
	// 自定义下拉组合框插件
	var FormPanel = function(ele, opt) {
		this.$parent = $(ele);
	...
	FormPanel.prototype = $.extend({
		...
		initializeForm:function(){
			...
			.attr('action', options.url || '#').attr('method', options.method || 'get');
			...
			// 添加字段
			this.appendFields($formTable);
			// 添加按钮
			if (this.settings.buttons) {
				this.createButtons($formTable);
			}
		},
		appendFields:function($formTable){
			var options = this.settings;
			var name = this.settings.name;
			
			for (var i = 0; i < options.items.length; i++) {
				var $tr = $("<div class='x-form-item'></div>").appendTo($formTable);
				this._fields[options.items[i].name]=this._appendField($tr, options.items[i]);
			}
		...
		_appendField : function($tr, item) {
			...
			if (item.xtype == 'text') {
				...
				$field = $fieldDiv.ComboBox(item);
			}else if(item.xtype == 'checkbox'){
				$field = $fieldDiv.CheckBox(item);
			}else if(item.xtype == 'radio'){
				$field = $fieldDiv.Radio(item);
			}else if(item.xtype == 'grid'){
				$field = $fieldDiv.GridPanel(item);
			}else if(item.xtype == 'property'){
				$field = $fieldDiv.PropertyGrid(item);
			}else{
		...
		// 添加按钮
		createButtons:function($formTable){
			var cols = this.settings.columns.length;
			var name = this.settings.name;
		...
		setItemValue:function(itemName){
	...
	$.fn.FormPanel = function(options) {
		...
		// 创建插件对象
		var comp = new FormPanel(this, opts);
...
Arvato.form.FormPanel.prototype = {
	...
	_appendField : function($tr,item) {
		...
		if (item.xtype == 'text') {
			...
			var $field = $(formatStr("<select name='{name}'></select>",item.name)).appendTo($fieldDiv).ComboBox(item);
			this.$fields[item.name]= $field.getComp();
